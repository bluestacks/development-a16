...
def get_token() -> str:
  ...
  try:
    with open(WINSCOPE_TOKEN_LOCATION, "r") as token_file:
      ...
      log.debug(
          "Loaded token {} from {}".format(token, WINSCOPE_TOKEN_LOCATION)
      )
      return token
  except IOError:
    ...
    try:
      with open(WINSCOPE_TOKEN_LOCATION, "w") as token_file:
        log.debug(
            "Created and saved token {} to {}".format(
...
class RequestRouter:
  ...
  def _bad_token(self):
    ...
    self.request.respond(
        ...
        b"Bad Winscope authorization token!\nThis is Winscope ADB proxy.\n",
        "text/txt",
    )
      ...
      try:
        ...
        if "RequestType" in repr(ex):
          return self._bad_request(
              "Unknown endpoint /{}/".format(endpoint_name)
          )
...
def call_adb(params: str, device: str = None):
  ...
  try:
    ...
    raise AdbError(
        "OS Error executing adb command: {}\n{}".format(command_str, repr(ex))
    )
  except subprocess.CalledProcessError as ex:
    return "Error executing adb command: {}: {}".format(
        command_str, ex.output.decode("utf-8")
    )
...
class TraceThread(threading.Thread):
  ...
  def __init__(
    ...
    try:
      ...
      raise AdbError(
          "Error executing adb command for trace {}: {}".format(
              target_id, repr(ex)
          )
      )
  ...
  def timeout(self):
    if self.is_alive():
      log.warning(
          "Keep-alive timeout for {} trace on {}".format(
  ...
  def reset_timer(self):
    log.info(
        "Resetting keep-alive clock for {} trace on {}".format(
            self.target_id, self._device_id
        )
    )
  ...
  def end_trace(self):
    if self._keep_alive_timer:
      self._keep_alive_timer.cancel()
    log.info(
        "Sending SIGINT to the {} process on {}".format(
            self.target_id, self._device_id
        )
    )
    ...
    try:
      log.debug(
          "Waiting for {} trace shell to exit for {}".format(
              self.target_id, self._device_id
          )
      )
      ...
      log.error(
          "TIMEOUT - sending SIGKILL to the {} trace process on {}".format(
              self.target_id, self._device_id
          )
      )
      self.process.kill()
  ...
  def run(self):
    ...
    log.info(
        "Trace {} ended on {}, waiting for cleanup".format(
            self.target_id, self._device_id
        )
    )
    time.sleep(0.2)
    for i in range(int(COMMAND_TIMEOUT_S / retry_interval)):
      if (
        ...
        log.info(
            "Trace {} finished on {}".format(self.target_id, self._device_id)
        )
        ...
        return
      log.debug(
          "Still waiting for cleanup on {} for {}".format(
              self._device_id, self.target_id
          )
      )
...
"""
  ...
  def process_with_device(self, server, path, device_id):
    ...
    thread = TraceThread(
        target_id, device_id, command.encode("utf-8"), status_filename
    )
...
class EndTraceEndpoint(DeviceRequestEndpoint):
  ...
  def process_with_device(self, server, path, device_id):
    ...
    if target_id not in threads:
      raise BadRequest(
          "No {} trace in progress for {}".format(target_id, device_id)
      )
    ...
    signal_handler_log = call_adb(
        f"shell cat {SIGNAL_HANDLER_LOG}", device=device_id
    ).encode("utf-8")
    ...
    if not success:
      ...
      errors.append(
          "Error ending trace {} on the device: {}".format(
              target_id, thread.err
          )
      )

    out = (
        b"### Shell script's stdout ###\n"
